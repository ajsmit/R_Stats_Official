---
title: "Inferences about one or two populations"
author: "AJ Smit"
date: "`r Sys.Date()`"
output:
...
![Significance](Resources/significance.jpeg){width="50%"}
...
![Wahlberg_assumptions](Resources/wahlberg_distribution.jpeg){width="50%"}
...
```{r t-test-plot1, fig.cap="Interactive histogram showing two randomly generated normal distributions."}
# Random normal data
set.seed(666)
...
```{r t-test-plot2, fig.cap="Boxplot of random normal data with. A hypothetical population mean of 20 is shown as a blue line, with the red line showing a mean of 30."}
...
```{r t-test-plot5, fig.cap="Boxplots showing differences in morphometric properties of the kelp _Ecklonia maxima_ at two sites in False Bay."}
...
```{r t-test-plot6, fig.cap="Boxplots showing the difference in stipe mass (kg) of the kelp _Ecklonia maxima_ at two sites in False Bay."}
# filter the data
...
> **Exercise 1** Use a suitable R function to generate samples of normal data. Think of a hypothesis you could use a t-test for. Write out the hypothetical hypothesis, test it, and write a one sentence conclusion for it. Provide all of the code used to accomplish this.

> **Exercise 2** Search for a built-in R dataset suitable for a t-test. Which t-test is appropriate for testing hypotheses about these data? Write down the hypothesis and test it.
