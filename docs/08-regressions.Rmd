...
<source type="video/mp4" src="https://ajsmit.github.io/R_Stats_Official//Resources/lm_rotate.avi">
...
The intercept is the best estimate of the starting point of the fitted line on the left-hand side of the graph. You will notice that there is also an estimate for the standard error of the estimate for the intercept.
...
We saw in the ANOVA chapter how we could add a categorical variable to a regression -- or rather, we saw how we could had a continuous co-variate to an ANOVA. Whichever way we look at it, the analysis is the same. This is called an Analysis of Covariance, or ANCOVA.
...
> **Exercise 1:** Please write a report on how to use diagnostic plots to show if the assumptions of normality and heteroscedasticity are being met in a regression analysis. The kinds of diagnostic plots I have in mind include, for example, graphs of Residuals vs Fitted values, Normal Q-Q plots, Scale-Location plots, and plots of Residuals vs Leverage. Produce the necessary graphs using built-in functions, and for two of them also construct them by hand (i.e. manually creating each of the steps that's included in tbe built-in functions, and then assembling the graphs from scratch). Apply the analysis on your own data (e.g. using the built-in datasets available within R), and discuss how to interpret them.
>
> **Hint:** One can apply the `plot()` function to the regression model object, or there are some functions in the **gg_diagnose** package. In fact, there are many, many ways of doing it.
...
<!-- ### Normal probability plot of residuals vs. theoretical quantiles (Normal Q-Q plot) -->

<!-- Q-Q plots identify the quantiles in your sample data and plot them against the quantiles of a theoretical distribution. For a normal distribution, the data points will fall along a straight diagonal line in a Q-Q plot. Although we tend to work with (or, at least, hope for) normal data in biology and ecology, other data distributions may also be encountered --- here too, we can use Q-Q plots to help decide if our data data plausibly comes from some other theoretical distribution, such as a binomial or exponential distribution. -->

<!-- To demonstrate, let us turn to the `Iris` dataset. I'll do the Q-Q plot calculation 'by hand' to demonstrate how it works. First, we need to calculate the *z*-scores (i.e., we subtract the mean of all the data in the sample from each observation, then we divide by the standard deviation of all the data in our sample). This is called *centering* (subtracting the mean, thus the sample will now assume a mean of `0`) and *scaling* (dividing by the SD, and the SD of the sample will become `1`): -->

```{r, echo=FALSE, eval=FALSE}
ex <- iris %>% 
  filter(Species == "setosa") %>% 
  select(Sepal.Length) %>%
  mutate(sl_std1 = (Sepal.Length - mean(Sepal.Length)) / sd(Sepal.Length)) %>% 
  as.vector()
```

<!-- The same might be achieved more easily through the use of the `scale()` function: -->

```{r, echo=FALSE, eval=FALSE}
ex <- ex %>% 
  mutate(sl_std2 = scale(Sepal.Length))

head(ex)
```

<!-- Let's check if the mean is `0` and the SD is `1`: -->
...
<!-- Now we rank them from small to large: -->
...
<!-- To make the Q-Q plot, we need to plot our standardised data (on *y*) versus data for an expected normal distribution (on *x*). We can create such data using the `rnorm()` function: -->

```{r, echo=FALSE, eval=FALSE}
ex <- ex %>% 
  mutate(x_dat = qnorm(sl_std1))

ggplot(data = ex, aes(x = x_dat, y = sl_std1)) +
  geom_point(col = "red3", shape = 10) +
  geom_smooth(method = "lm", col = "black", se = FALSE) +
  labs(x = "Theoretical quantiles", y = "Sample quantiles")
```

<!-- But don't fret. There is an easy way to do this: -->

```{r, echo=FALSE, eval=FALSE}
library("car")
qqPlot(ex$Sepal.Length)
```
